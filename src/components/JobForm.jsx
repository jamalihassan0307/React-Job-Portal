...
const JobForm = ({ onSubmit, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setSalary(initialData.salary || "Under $50K");
  ...
  const funSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(initialData ? 'Failed to update job' : 'Failed to add job');
    }
  };
  ...
  return (
    <form onSubmit={funSubmit}>
      <div className="mb-4">
        <label htmlFor="type" className="block text-YELLOW font-bold mb-2">
          Job Type
        </label>
        <select
          id="type"
